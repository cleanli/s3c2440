...
dm9000_probe(void)
{
	...
	if (id_val == DM9000_ID) {
		lprintf("dm9000 i/o: 0x%x, id: 0x%x \n", CONFIG_DM9000_BASE,
		       id_val);
		return 0;
	} else {
		lprintf("dm9000 not found at 0x%08x id: 0x%08x\n",
...
dm9000_reset(void)
{
	...
	if ((DM9000_ior(DM9000_PIDL) != 0x0) ||
		...
		lprintf("ERROR: resetting DM9000 -> not responding\n");
}
...
static int dm9000_init(struct eth_device *dev, bd_t *bd)
{
	...
	case 0x0:  /* 16-bit mode */
		lprintf("DM9000: running in 16 bit mode\n");
		db->outblk    = dm9000_outblk_16bit;
		db->inblk     = dm9000_inblk_16bit;
		db->rx_status = dm9000_rx_status_16bit;
		break;
	case 0x01:  /* 32-bit mode */
		lprintf("DM9000: running in 32 bit mode\n");
		db->outblk    = dm9000_outblk_32bit;
		db->inblk     = dm9000_inblk_32bit;
		db->rx_status = dm9000_rx_status_32bit;
		break;
	case 0x02: /* 8 bit mode */
		lprintf("DM9000: running in 8 bit mode\n");
	...
	default:
		...
		lprintf("DM9000: Undefined IO-mode:0x%x\n", io_mode);
		db->outblk    = dm9000_outblk_8bit;
		db->inblk     = dm9000_inblk_8bit;
		db->rx_status = dm9000_rx_status_8bit;
		break;
	...
	while (!(phy_read(1) & 0x20)) {	/* autonegation complete bit */
		...
		i++;
		if (i == 1000) {
			lprintf("could not establish link\n");
			return 0;
		}
	}
	...
	/* see what we've got */
	lnk = phy_read(17) >> 12;
	lprintf("operating at ");
	switch (lnk) {
	case 1:
		lprintf("10M half duplex ");
		break;
	case 2:
		lprintf("10M full duplex ");
		break;
	case 4:
		lprintf("100M half duplex ");
		break;
	case 8:
		lprintf("100M full duplex ");
		break;
	default:
		lprintf("unknown: %d ", lnk);
		break;
	}
	lprintf("mode\n");
#endif
...
static int dm9000_rx(struct eth_device *netdev, volatile void * rcv_buf)
{
	...
	for (;;) {
		...
		if (rxbyte > DM9000_PKT_RDY) {
			...
			lprintf("DM9000 error: status check fail: 0x%x\n",
				rxbyte);
			return 0;
		}
		...
		if ((RxStatus & 0xbf00) || (RxLen < 0x40)
			...
			if (RxStatus & 0x100) {
				lprintf("rx fifo error\n");
			}
			if (RxStatus & 0x200) {
				lprintf("rx crc error\n");
			}
			if (RxStatus & 0x8000) {
				lprintf("rx length error\n");
			}
			if (RxLen > DM9000_PKT_MAX) {
				lprintf("rx length too big\n");
				dm9000_reset();
			}
		} else {
